...
export function Chat() {
  ...
  const formatContent = (content: string) => {
    ...
    return content
      ...
      .map((line) => {
        ...
        // Headers
        if (line.startsWith('### ')) return `<h4 class="font-semibold text-slate-900 mt-3 mb-1">${line.slice(4)}</h4>`;
        if (line.startsWith('## ')) return `<h3 class="font-semibold text-slate-900 text-lg mt-4 mb-2">${line.slice(3)}</h3>`;
        // List items
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-120px)] bg-white rounded-xl border border-slate-200 overflow-hidden">
      {/* Header */}
      <div className="px-6 py-4 border-b border-slate-200 bg-gradient-to-r from-primary-50 to-blue-50">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="font-semibold text-slate-900">EdgeAI Assistant</h2>
            <p className="text-sm text-slate-500">Powered by IIM → CIM → BIM pipeline</p>
          </div>
        </div>
      </div>
            ...
            <div className="flex flex-wrap justify-center gap-2 max-w-2xl">
              {QUICK_PROMPTS.map((prompt, i) => (
                <button
                  ...
                  className="px-4 py-2 text-sm bg-white border border-slate-200 text-slate-700 rounded-full hover:bg-blue-50 hover:border-blue-200 hover:text-blue-700 transition-all shadow-sm"
                >
                  {prompt}
                </button>
              ))}
            ...
            {messages.map((message) => (
              ...
              >
                ...
                <div className={`max-w-[70%] ${message.role === 'user' ? 'order-first' : ''}`}>
                  ...
                  <p className={`text-xs mt-1 ${message.role === 'user' ? 'text-right text-slate-500' : 'text-slate-500'}`}>
      ...
      <div className="px-6 py-4 border-t border-slate-200 bg-slate-50">
        <form onSubmit={handleSubmit} className="flex gap-3">
          <input
            ...
            className="flex-1 px-4 py-3 border border-slate-300 rounded-xl focus:ring-2 focus:ring-primary-500 focus:border-primary-500 outline-none transition-colors"
            disabled={isLoading}
          />
          <button
            type="submit"
          ...
          <div className="flex gap-2 mt-3 overflow-x-auto pb-1">
            {QUICK_PROMPTS.slice(0, 4).map((prompt, i) => (
              <button
                ...
                onClick={() => handleQuickPrompt(prompt)}
                disabled={isLoading}
                className="px-3 py-1.5 text-xs bg-white border border-slate-200 hover:bg-blue-50 hover:border-blue-200 hover:text-blue-700 text-slate-600 rounded-full whitespace-nowrap transition-all disabled:opacity-50"
              >
                {prompt}
              </button>
            ))}
