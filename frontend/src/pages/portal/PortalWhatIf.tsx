    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-6xl mx-auto px-4 py-6 space-y-6">
        <div>
          ...
          <p className="text-gray-500 text-sm">Explore how different choices affect your retirement outlook</p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-1 space-y-4">
            <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5 space-y-5">
              <h2 className="font-semibold text-gray-900 flex items-center gap-2"><Calculator className="h-5 w-5 text-blue-600" /> Parameters</h2>
              ...
              <div>
                ...
                <p className="text-lg font-semibold text-gray-900">{fmtCur(savings)}</p>
                <p className="text-xs text-gray-400">From your current portfolio</p>
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="number" value={monthly} onChange={(e) => setMonthly(Number(e.target.value))} className="w-full pl-7 pr-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500" />
                </div>
              </div>

              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Expected Return</label>
                <select value={returnRate} onChange={(e) => setReturnRate(Number(e.target.value))} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
              ...
              <div>
                ...
                <select value={inflation} onChange={(e) => setInflation(Number(e.target.value))} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                  {[2, 2.5, 3, 3.5].map((r) => <option key={r} value={r}>{r}%</option>)}
                </select>
              </div>

              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Monthly Spending in Retirement</label>
                <div className="relative">
                  <span className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500 text-sm">$</span>
                  <input type="number" value={spending} onChange={(e) => setSpending(Number(e.target.value))} className="w-full pl-7 pr-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500" />
                </div>
              </div>
              ...
              <div className="flex gap-2 pt-2">
                ...
                <button onClick={handleReset} className="px-3 py-2.5 text-gray-500 hover:bg-gray-100 rounded-lg transition-colors" title="Reset"><RotateCcw className="h-4 w-4" /></button>
              </div>
            </div>
          </div>
            ...
            {p && (
              <>
                ...
                <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
                  <h3 className="font-semibold text-gray-900 mb-4">Projected Growth</h3>
                  <div className="h-72">
                ...
                {result?.comparison && result.comparison.length > 0 && (
                  <div>
                    <h3 className="font-semibold text-gray-900 mb-3">Retirement Age Comparison</h3>
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                      {result.comparison.map((c) => {
                        const isSelected = c.retirement_age === retireAge;
                        return (
                          <div key={c.retirement_age} className={`bg-white rounded-xl border shadow-sm p-5 ${isSelected ? 'border-blue-500 ring-2 ring-blue-100' : 'border-gray-200'}`}>
                            <div className="flex items-center justify-between mb-3">
                              <span className="text-sm font-medium text-gray-500">Retire at {c.retirement_age}</span>
                              {isSelected && <span className="text-xs bg-blue-100 text-blue-700 px-2 py-0.5 rounded-full font-medium">Selected</span>}
                            </div>
                            <p className="text-xl font-bold text-gray-900">{fmtCur(c.balance)}</p>
                            <div className="mt-3 space-y-1.5 text-sm">
                              <div className="flex justify-between"><span className="text-gray-500">Monthly Income</span><span className="font-medium">{fmtCur(c.monthly_income)}</span></div>
                              <div className="flex justify-between"><span className="text-gray-500">Income Years</span><span className="font-medium">{c.years_of_income}</span></div>
