    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-5xl mx-auto px-4 py-6 space-y-6">
        ...
        <div>
          ...
          <p className="text-gray-500 text-sm">Household overview and linked family members</p>
        </div>
          ...
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
            <div>
              <div className="flex items-center gap-3 mb-3">
                ...
                <h2 className="text-xl font-bold text-gray-900">{data.household_name}</h2>
              </div>
              <p className="text-3xl font-bold text-gray-900">{fmtCur(data.total_household_value)}</p>
              <p className="text-sm text-gray-500 mt-1">{totalMembers} member{totalMembers !== 1 ? 's' : ''} &middot; {totalAccounts} accounts</p>
            </div>

            {/* Allocation */}
            <div className="w-full lg:w-72 space-y-3">
              <h3 className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Household Allocation</h3>
              {/* Stacked bar */}
            ...
            {data.members.map((m) => (
              <div
                ...
                className={`bg-white rounded-xl border shadow-sm overflow-hidden transition-all ${
                  m.can_view_details ? 'hover:shadow-md cursor-pointer border-gray-200' : 'border-gray-200 opacity-80'
                }`}
              ...
              >
                <div className="p-5">
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex items-center gap-3">
                      <div className={`w-10 h-10 rounded-full flex items-center justify-center text-sm font-semibold ${
                        m.is_self ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-600'
                      }`}>
                  ...
                  <div className="grid grid-cols-3 gap-3 text-center">
                    <div>
                      <p className="text-lg font-bold text-gray-900">{fmtCur(m.total_value)}</p>
                    ...
                    <div>
                      <p className="text-lg font-bold text-gray-900">{m.accounts_count}</p>
        ...
        {dependents.length > 0 && (
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {dependents.map((dep) => (
                <div key={dep.name} className="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-500">{dep.relationship} &middot; Age {dep.age}</p>
                    </div>
                  </div>
                  {dep.has_529 ? (
                    <div className="flex items-center justify-between p-3 bg-purple-50 rounded-lg">
                      <div>
                        <p className="text-xs text-gray-500">529 Education Plan</p>
                        <p className="text-lg font-bold text-gray-900">{fmtCur(dep.plan_value)}</p>
                      </div>
                    ...
                    <div className="p-3 bg-gray-50 rounded-lg text-center">
                      <p className="text-sm text-gray-500">No 529 plan</p>
              ...
              {jointAccounts.map((ja, i) => (
                <div key={i} className="flex items-center gap-4 p-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-gray-900">{ja.name}</p>
                    <p className="text-xs text-gray-500">{ja.type}</p>
                  </div>
                  <p className="text-sm font-bold text-gray-900">{fmtCur(ja.value)}</p>
                </div>
              ))}
            </div>
          </div>
            ...
            {loadingMember ? (
              ...
              <>
                <div className="flex items-center justify-between p-5 border-b border-gray-100">
                  <div className="flex items-center gap-3">
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center text-sm font-semibold ${
                      selectedMember.is_self ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-600'
                    }`}>
                  ...
                  <button onClick={() => setSelectedMember(null)} className="text-gray-400 hover:text-gray-600"><X className="h-5 w-5" /></button>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4 p-5 border-b border-gray-100">
                  <div>
                    <p className="text-xs text-gray-500">Total Value</p>
                    <p className="text-xl font-bold text-gray-900">{fmtCur(selectedMember.total_value)}</p>
                  </div>
                  <div>
                    <p className="text-xs text-gray-500">YTD Return</p>
                ...
                {selectedMember.accounts && selectedMember.accounts.length > 0 && (
                  <div className="p-5 space-y-3">
                    <h4 className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Accounts</h4>
                    {selectedMember.accounts.map((a, i) => (
                      <div key={i} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                        ...
                        <p className="text-sm font-bold text-gray-900">{fmtCur(a.value)}</p>
                      </div>
                    ))}
                  </div>
                )}
